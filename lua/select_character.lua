...
-- 20260524221732 当候选字为 4 以上时快捷键让字上屏，可以在 default.yaml 中修改
...
function select.init(env)
    ...
    env.max_chars = tonumber(config:get_string('key_binder/select_max_chars')) or 4
end
...
function select.func(key, env)
    ...
    then
        ...
        if utf8.len(text) > env.max_chars then
            return 2
