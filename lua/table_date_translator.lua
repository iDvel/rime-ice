...
"日期", --表的第二个键值,必填,是本日期格式组的默认提示码,只作用于这部分日期格式
 {'%Y-%m-%d")',false}, --表的第三个及以后键值,必填,代表输入编码时所输出的日期格式
 {'%Y/%m/%d")',false}, --从此开始,所有键值都是选填项,因为你至少得输出一个时间格式才会需要这个脚本
 {'%Y.%m.%d")',false}, --'%Y-%m-%d'代表日期格式:年-月-日
 {'%Y%m%d")',false}}, --false代表特殊提示码,只作用与这一个日期格式,可以设置为false、nil和任意字符串,设置为空字符串""可以关闭这个日期格式的提示码,false和nil代表不设置特殊提示码
--紧凑易读的写法,建议按此格式添加新的日期格式
 {"time","时间",
 {'%H:%M")',false},
 {'%H:%M:%S")',false}}, --收尾要两个右花括号

 {"time2","时间2",{%H:%M:%S")',false}},--如果你只添加一个日期格式,可以将四项写在一行

 {"time3","时间3",{%H:%M:%S")',false}},--最后一组日期格式不需要逗号分隔
 ...
 20230614003838 由蹄垫hoofcushion编辑
}
--]]
w={"日", "一", "二", "三", "四", "五", "六"}
local t={"",
