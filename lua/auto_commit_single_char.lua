...
function P.init(env)
	P.char = env.engine.schema.config:get_string(env.name_space:gsub('^*', ''))
end
...
function P.func(key, env)
	...
	-- 不影响组合键
	if not P.char or key:release() or key:ctrl() or key:alt() or key:super() or key:shift() then
		return 2
	end
	...
	if not context:is_composing() and ascii_str == P.char
	then
		return 0 -- kRejected
	end
